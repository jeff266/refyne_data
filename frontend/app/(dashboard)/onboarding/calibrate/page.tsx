...
export default function CalibratePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isRecalibrate) {
        router.push('/settings?tab=general&success=calibration-updated');
      } else {
        router.push('/normalize?onboarding=complete');
      }
    } catch (err) {
        ...
        <>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button
                onClick={() => router.push(isRecalibrate ? '/settings' : '/dashboard')}
                style={{
                  padding: '8px 16px',
                  background: C.red,
                  border: 'none',
