...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (bullmqJobId) {
      ...
      // Start timeout guard (fail scans stuck for >10 minutes)
      setTimeout(async () => {
        if (!supabase) return;
        ...
        if (stuckJob && stuckJob.status === 'scanning') {
          console.error(`[Scan Job] Timeout guard: scan ${job.id} stuck for >10 minutes, marking as failed`);
          await supabase
            .from('harmony_scan_jobs')
            .update({
              status: 'failed',
              error_message: 'Scan timeout: no progress for 10 minutes. Please try again or contact support.',
    ...
    // BullMQ queue not available - fail explicitly
    console.error('[Scan Job] BullMQ queue not available - cannot process scan');
    await supabase
      .from('harmony_scan_jobs')
      .update({
        status: 'failed',
        error_message: 'Worker queue unavailable. Please contact support.',
        completed_at: new Date().toISOString(),
      })
      .eq('id', job.id);

    return NextResponse.json(
      { error: 'Scan worker unavailable. Please try again later.' },
      { status: 503 }
    );
  } catch (error: any) {
